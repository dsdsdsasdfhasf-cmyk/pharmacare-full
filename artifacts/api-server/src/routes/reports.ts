...
router.get("/reports/profit-loss", async (req, res) => {
  ...
  const rows = await db
    .select({
      ...
      totalSales: sql<number>`count(distinct ${salesTable.id})::int`,
      totalItems: sql<number>`sum(${saleItemsTable.quantity})::int`,
    })
    .from(saleItemsTable)
...
router.get("/reports/profit-by-medicine", async (_req, res) => {
  const rows = await db
    .select({
      ...
      totalQuantitySold: sql<number>`sum(${saleItemsTable.quantity})::int`,
