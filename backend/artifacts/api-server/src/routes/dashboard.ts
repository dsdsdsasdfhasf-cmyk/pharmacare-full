...
router.get("/dashboard/summary", async (_req, res) => {
  ...
  const [todaySalesData] = await db
    .select({ count: sql<number>`count(*)`, revenue: sql<number>`coalesce(sum(total_amount), 0)` })
    .from(salesTable)
  ...
  const [totalMedicines] = await db.select({ count: sql<number>`count(*)` }).from(medicinesTable);
  const [lowStock] = await db.select({ count: sql<number>`count(*)` }).from(medicinesTable).where(sql`quantity <= min_quantity`);
  const [expiringSoon] = await db.select({ count: sql<number>`count(*)` }).from(medicinesTable).where(
    and(
  ...
  const [totalCustomers] = await db.select({ count: sql<number>`count(*)` }).from(customersTable);
...
router.get("/dashboard/sales-chart", async (req, res) => {
  ...
  if (period === "today") {
    rows = await db
      .select({
        date: sql<string>`strftime('%H:00', ${salesTable.createdAt}, 'unixepoch')`,
      ...
      .groupBy(sql`strftime('%H:00', ${salesTable.createdAt}, 'unixepoch')`)
      .orderBy(sql`strftime('%H:00', ${salesTable.createdAt}, 'unixepoch')`);
  } else {
    rows = await db
      .select({
        date: sql<string>`date(${salesTable.createdAt}, 'unixepoch')`,
        revenue: sql<number>`coalesce(sum(total_amount), 0)`,
        salesCount: sql<number>`count(*)`,
      })
      .from(salesTable)
...
router.get("/dashboard/top-medicines", async (req, res) => {
  ...
  const rows = await db
    .select({
      ...
      totalQuantity: sql<number>`sum(${saleItemsTable.quantity})`,
...
router.get("/dashboard/weekly-comparison", async (_req, res) => {
  ...
  const [thisWeek] = await db
    .select({ revenue: sql<number>`coalesce(sum(total_amount), 0)`, count: sql<number>`count(*)` })
    .from(salesTable)
